...
import { driverCreateSchema } from "@/modules/roster/wire";
...
export async function GET() {
  ...
  return Response.json(await listDrivers(getDb()));
}
...
export async function POST(req: Request) {
  ...
  if (!parsed.success) {
    return errorResponse("VALIDATION_FAILED", "That driver is not valid.", {
      issues: parsed.error.flatten().fieldErrors,
    });
  }

  const result = await createDriver(getDb(), parsed.data, {
